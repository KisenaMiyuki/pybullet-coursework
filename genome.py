...
class Genome():
    ...
    def crossover(g1, g2):
        """
        Generates a new genome by combining some portion of g1 and some portion of g2 randomly.
    ...
    def point_mutate(genome, rate, amount):
        """
        Mutates properties of gene in the input genome by adding 0.1 to it,
        while clamping it to 0~1

        :param genome: The genome to mutate
        :param rate: [0~1] The probability of mutation (of individual properties in each gene in input genome), lower value means lower probability of mutation
    ...
    def shrink_mutate(genome, rate):
        """
        Mutates the input genome by randomly removing one of its genes according to rate.
        May not remove anything, but at max only remove 1 gene.
        Do nothing if input genome only has 1 gene (genome has length of 1).

        :param genome: The genome to mutate
        :param rate: [0~1] The probability of mutation, lower value means lower probability of mutation
    ...
    def grow_mutate(genome, rate):
        """
        Mutates the input genome by randomly adding a new gene to input genome according to rate.
        May not add anything, but at max only add 1 gene.
        ...
        """
        if random.random() < rate:
            # generate new gene which length is equal to first gene in genome
