...
def main(csv_file, environment_type):
    ...
    # ========== Use existing Simulation class Method ==========
    p.disconnect()
    sim = simulation_v2.Simulation(sim_id=0, sim_mode="gui")
    sim.connect()
    sim.run_creature_from_csv(csv_file, env_id=1, environment_config=ENVIRONMENT_CONFIG, iterations=CREATURE_SIMULATION_ITERATIONS)
    sim.disconnect()
    ...
    # rob1 = p.loadURDF('test.urdf')  # creature_id
    #
    # # air drop it
    ...
    #         if step > CREATURE_SIMULATION_ITERATIONS:
    #             break
    #
...
if __name__ == "__main__":
    assert len(sys.argv) == 3, "Usage: python realtime_from_csv.py csv_filename environment_type(0-plane, 1-mountain)"
